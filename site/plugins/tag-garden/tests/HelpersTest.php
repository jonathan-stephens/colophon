 ...
 * Helper Class Tests
 *
 * Tests the TagGarden\Helpers static class methods
...
class HelpersTest {
    ...
    public function runAll() {
        ...
        if (!class_exists('TagGarden\Helpers')) {
            echo "<p style='color: #721c24; background: #f8d7da; padding: 15px; border-radius: 5px;'>";
            echo "❌ TagGarden\Helpers class not found. Make sure lib/Helpers.php is loaded.";
            echo "</p>";
            return false;
        }
    ...
    private function testGetGrowthDefinition() {
        // Test valid status
        $seedling = \TagGarden\Helpers::getGrowthDefinition('seedling');
        $this->assert(
        ...
        // Test invalid status
        $invalid = \TagGarden\Helpers::getGrowthDefinition('invalid-status');
        $this->assert(
    ...
    private function testGetGroupDefinition() {
        // Test valid group
        $garden = \TagGarden\Helpers::getGroupDefinition('garden');
        $this->assert(
        ...
        foreach ($groups as $group) {
            $def = \TagGarden\Helpers::getGroupDefinition($group);
            $this->assert(
        ...
        // Test invalid group
        $invalid = \TagGarden\Helpers::getGroupDefinition('invalid-group');
        $this->assert(
    ...
    private function testGetThemeDefinition() {
        // Test valid theme
        $topic = \TagGarden\Helpers::getThemeDefinition('topic');
        $this->assert(
        ...
        foreach ($themes as $theme) {
            $def = \TagGarden\Helpers::getThemeDefinition($theme);
            $this->assert(
    ...
    private function testGetLengthCategory() {
        ...
        foreach ($tests as $test) {
            $result = \TagGarden\Helpers::getLengthCategory($test['count']);
            $this->assert(
    ...
    private function testGetLengthLabel() {
        ...
        foreach ($categories as $category) {
            $label = \TagGarden\Helpers::getLengthLabel($category);
            $this->assert(
    ...
    private function testFormatReadingTime() {
        ...
        $result = \TagGarden\Helpers::formatReadingTime($timeData);
        $this->assert(
    ...
    private function testGetSortMethods() {
        $methods = \TagGarden\Helpers::getSortMethods();
    ...
    private function testGetTagFontSize() {
        // Test with range
        $minSize = \TagGarden\Helpers::getTagFontSize(1, 1, 10);  // Minimum usage
        $maxSize = \TagGarden\Helpers::getTagFontSize(10, 1, 10); // Maximum usage
        $midSize = \TagGarden\Helpers::getTagFontSize(5, 1, 10);  // Middle usage
        ...
        $sameSize = \TagGarden\Helpers::getTagFontSize(5, 5, 5);
        $this->assert(
    ...
    private function testSanitizeTag() {
        ...
        foreach ($tests as $test) {
            $result = \TagGarden\Helpers::sanitizeTag($test['input']);
            $this->assert(
    ...
    private function testTagsToUrl() {
        ...
        $result = \TagGarden\Helpers::tagsToUrl($tags);
        ...
        $single = \TagGarden\Helpers::tagsToUrl(['design']);
        $this->assert(
    ...
    private function testUrlToTags() {
        ...
        $result = \TagGarden\Helpers::urlToTags($urlString);
        ...
        // Test with single tag
        $single = \TagGarden\Helpers::urlToTags('design');
        $this->assert(
    ...
    private function testGetAllTags() {
        $tags = \TagGarden\Helpers::getAllTags();
    ...
    private function testGetPagesByTags() {
        $pages = \TagGarden\Helpers::getPagesByTags('design');
        ...
        // Test with array
        $pagesArray = \TagGarden\Helpers::getPagesByTags(['design', 'web']);
        $this->assert(
        ...
        // Test empty input
        $emptyPages = \TagGarden\Helpers::getPagesByTags([]);
        $this->assert(
    ...
    private function testSortPages() {
        ...
        foreach ($methods as $method) {
            $sorted = \TagGarden\Helpers::sortPages($pages, $method);
            $this->assert(
