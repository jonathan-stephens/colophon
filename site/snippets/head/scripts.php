  <!-- Scripts -->
    ...
    // Service Worker
      if('serviceWorker'in navigator){window.addEventListener('load',()=>{navigator.serviceWorker.register('/sw.js',{scope:'/',updateViaCache:'none'}).then(r=>{r.addEventListener('updatefound',()=>{const installing=r.installing;installing?.addEventListener('statechange',()=>{if(installing.state==='installed'&&navigator.serviceWorker.controller){console.log('Update available - refresh to activate')}})})}).catch(e=>console.error('SW registration failed:',e));let prevController=navigator.serviceWorker.controller;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(prevController){window.location.reload()}prevController=navigator.serviceWorker.controller})})}
    </script>
